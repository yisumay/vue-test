...
function genTextNode(token, elementStack) {
  const parent = elementStack[elementStack.length - 1]
   ...
   function parse(str) {
    ...
    console.log('%c [ 生成的token如下 ]', 'font-size:13px; background:pink; color:#bf2c9f;', tokens);
    // 创建 Root 根节点
    const root = {
      type: 'Root',
      children: []
  ...
  function genAst (target) {
    ...
    console.log('%c [ 生成的AST树如下 ]', 'font-size:13px; background:pink; color:#bf2c9f;', ast);
  }
